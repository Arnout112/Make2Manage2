...
export default function CapacityPlanner({
  ...
  const departmentCapacities = useMemo((): DepartmentCapacity[] => {
    return gameState.departments.map((dept) => {
      ...
      const predictedUtilization = Math.min(
        100,
        currentUtilization + (queueProcessingTime / (maxCapacity * 60)) * 100
      );
  ...
  const workloadRebalancing = useMemo((): WorkloadRebalancingPlan | null => {
    const overloadedDepts = departmentCapacities.filter(
      (d) => d.currentUtilization > 85
    );
    const underloadedDepts = departmentCapacities.filter(
      (d) => d.currentUtilization < 50 && d.availableCapacity > 0
    );

    if (overloadedDepts.length === 0 || underloadedDepts.length === 0)
      return null;
    ...
    overloadedDepts.forEach((dept) => {
      const deptOrders =
        gameState.departments.find((d) => d.id === dept.id)?.queue.slice(-2) ||
        [];
    ...
    return {
      ...
      expectedImprovements: {
        ...
        balancedUtilization:
          departmentCapacities.reduce(
            (sum, d) => sum + d.currentUtilization,
            0
          ) / departmentCapacities.length,
      },
    };
      ...
      <div className="p-6 border-b border-gray-200">
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-sm text-gray-500">
            R04-R05: Order Scheduling & Capacity
          </span>
        </div>
            ...
            {departmentCapacities.map((dept) => (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-medium ${getBottleneckColor(
                      dept.bottleneckRisk
                    )}`}
                  >
                  ...
                  <div>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${Math.min(dept.currentUtilization, 100)}%`,
                        }}
                      />
                    </div>
                  </div>
                    ...
                    <div className="border-t border-gray-200 pt-2 mt-2">
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div>
                          ...
                          <p className="font-medium">
                            {dept.predictedUtilization.toFixed(1)}%
                          </p>
                        </div>
                        <div>
                          <span className="text-gray-600">Optimal:</span>
                          <p className="font-medium">
                            {dept.optimalLoad} units
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {workloadRebalancing.sourceIds.map((id) => {
                      const dept = departmentCapacities.find(
                        (d) => d.id === id
                      );
                      return (
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  <div className="text-center">
                    <div className="text-lg font-bold text-blue-600">
                      -
                      {
                        workloadRebalancing.expectedImprovements
                          .reducedBottlenecks
                      }
                    </div>
                  ...
                  <div className="text-center">
                    <div className="text-lg font-bold text-purple-600">
                      {workloadRebalancing.expectedImprovements.balancedUtilization.toFixed(
                        1
                      )}
                      %
                    </div>
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <div className="text-center p-4 bg-yellow-50 rounded-lg">
              ...
              <div className="text-2xl font-bold text-yellow-600">
                {
                  departmentCapacities.filter(
                    (d) => d.bottleneckRisk === "high"
                  ).length
                }
              </div>
