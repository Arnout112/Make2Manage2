...
export default function RouteOptimizer({
  ...
  const departmentMetrics = useMemo(() => {
    return departments.reduce((acc, dept) => {
      ...
      acc[dept.id] = {
        ...
        riskScore:
          utilization > 0.8 ? "high" : utilization > 0.6 ? "medium" : "low",
      };
  ...
  const generateAlternativeRoutes = (
    ...
    if (originalRoute.length > 3) {
      ...
      if (
        parallelRoute.length !== originalRoute.length ||
        !arraysEqual(parallelRoute, originalRoute)
      ) {
  ...
  const optimizeForParallelProcessing = (route: number[]): number[] => {
    ...
    for (let i = 0; i < optimized.length - 1; i++) {
      ...
      if (
        departmentMetrics[current]?.utilization >
        departmentMetrics[next]?.utilization + 0.2
      ) {
  ...
  const optimizeForLoadBalancing = (route: number[]): number[] => {
    ...
    return route.map((deptId) => {
      ...
      if (deptMetric?.utilization > 0.8) {
        ...
        const alternatives = departments.filter(
          (d) =>
            d.id !== deptId &&
            departmentMetrics[d.id]?.utilization < 0.6 &&
            d.operations.some((op) =>
              departments
                .find((orig) => orig.id === deptId)
                ?.operations.some(
                  (origOp) =>
                    op.name.includes(origOp.name.split(" ")[0]) ||
                    origOp.name.includes(op.name.split(" ")[0])
                )
            )
        );
        ...
        if (alternatives.length > 0) {
          ...
          return alternatives.reduce((best, current) =>
            departmentMetrics[current.id].availabilityScore >
            departmentMetrics[best.id].availabilityScore
              ? current
              : best
          ).id;
        }
  ...
  const analyzeRoute = (route: number[]): RouteOption => {
    ...
    const efficiency =
      route.reduce(
        (acc, deptId) => acc + (departmentMetrics[deptId]?.efficiency || 1),
        0
      ) / route.length;
  ...
  const analyzeOrderRoute = (order: Order): RouteAnalysis => {
    ...
    const bestAlternative = alternatives.reduce((best, current) => {
      const bestScore =
        best.efficiency * 0.4 +
        (100 - best.utilization) * 0.3 +
        (best.estimatedTime > 0 ? (1000 / best.estimatedTime) * 0.3 : 0);
      const currentScore =
        current.efficiency * 0.4 +
        (100 - current.utilization) * 0.3 +
        (current.estimatedTime > 0 ? (1000 / current.estimatedTime) * 0.3 : 0);
      return currentScore > bestScore ? current : best;
    }, alternatives[0]);

    if (bestAlternative) {
      const timeSaving =
        originalOption.estimatedTime - bestAlternative.estimatedTime;
      const costSaving = originalOption.cost - bestAlternative.cost;
      const riskReduction =
        originalOption.riskLevel !== "low" &&
        bestAlternative.riskLevel === "low";

      if (timeSaving > 10 || costSaving > 50 || riskReduction) {
        recommendation = "use-alternative";
        if (timeSaving > 10)
          reasoning.push(
            `Saves ${timeSaving.toFixed(1)} minutes processing time`
          );
    ...
    if (originalOption.bottleneckDepartments.length > 0) {
      reasoning.push(
        `Bottlenecks detected in departments: ${originalOption.bottleneckDepartments.join(
          ", "
        )}`
      );
  ...
  const getRecommendationIcon = (rec: string) => {
    switch (rec) {
      ...
      case "optimize-further":
        return <AlertTriangle className="text-yellow-600" size={20} />;
      default:
        return <Route className="text-gray-600" size={20} />;
    }
  };
          ...
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="text-lg font-semibold text-gray-800">
                Route Analysis for {selectedOrder?.id}
              </h4>
            </div>
              ...
              <div className="flex items-center gap-4 mb-3">
                ...
                <span
                  className={`px-2 py-1 rounded-full text-xs font-medium ${getRiskColor(
                    routeAnalysis.originalRoute.riskLevel
                  )}`}
                >
              ...
              <div className="grid grid-cols-4 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">
                    {routeAnalysis.originalRoute.estimatedTime.toFixed(1)}m
                  </p>
                </div>
                <div>
                  <span className="text-gray-600">Est. Cost:</span>
                  <p className="font-medium">
                    ${routeAnalysis.originalRoute.cost}
                  </p>
                </div>
                <div>
                  <span className="text-gray-600">Avg. Utilization:</span>
                  <p className="font-medium">
                    {routeAnalysis.originalRoute.utilization.toFixed(1)}%
                  </p>
                </div>
                <div>
                  <span className="text-gray-600">Efficiency:</span>
                  <p className="font-medium">
                    {routeAnalysis.originalRoute.efficiency.toFixed(2)}x
                  </p>
                </div>
              </div>
            </div>
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-4">
                        ...
                        {alt.estimatedTime <
                          routeAnalysis.originalRoute.estimatedTime && (
                      ...
                      <button
                        onClick={() =>
                          onSelectAlternativeRoute?.(selectedOrderId, index)
                        }
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        ...
                        <div>
                          ...
                          <p className="font-medium">
                            {(metrics.condition * 100).toFixed(1)}%
                          </p>
                        </div>
                      </div>
                    </div>
