...
export default function PerformanceDashboard({
  ...
  const [selectedView, setSelectedView] = useState<
    "overview" | "detailed" | "trends" | "benchmarks" | "insights"
  >("overview");
  const [selectedTimeframe, setSelectedTimeframe] = useState<
    "current" | "last-hour" | "session" | "all-time"
  >("current");
  ...
  const kpiMetrics = useMemo((): KPIMetric[] => {
    ...
    const avgUtilization =
      gameState.departments.length > 0
        ? gameState.departments.reduce((sum, d) => sum + d.utilization, 0) /
    ...
    const sessionTimeHours = gameState.session.elapsedTime
      ? gameState.session.elapsedTime / (1000 * 60 * 60)
      : 1;
    ...
    const rushOnTime = rushOrders.filter(
      (o) => o.status === "completed-on-time"
    ).length;
    const customerSatisfaction =
      rushOrders.length > 0 ? (rushOnTime / rushOrders.length) * 100 : 85;

    const getStatus = (
      value: number,
      thresholds: { poor: number; average: number; excellent: number },
      reverse = false
    ) => {
      if (reverse) {
    ...
    return [
      {
        ...
        trend:
          onTimeRate > benchmarks.competitorAverages.onTimeDeliveryRate
            ? "up"
            : "down",
        trendValue: Math.abs(
          onTimeRate - benchmarks.competitorAverages.onTimeDeliveryRate
        ...
        recommendation:
          onTimeRate < 85
            ? "Focus on improving production scheduling and capacity planning"
            : undefined,
      },
      {
        ...
        trend:
          avgUtilization > benchmarks.competitorAverages.utilizationRate
            ? "up"
            : "down",
        trendValue: Math.abs(
          avgUtilization - benchmarks.competitorAverages.utilizationRate
        ...
        recommendation:
          avgUtilization < 70
            ? "Consider workload rebalancing and capacity optimization"
            : avgUtilization > 90
            ? "High utilization detected - monitor for bottlenecks"
            : undefined,
      },
      {
        id: "lead-time",
        ...
        trend:
          avgLeadTime < benchmarks.competitorAverages.leadTime ? "up" : "down",
        trendValue: Math.abs(
          avgLeadTime - benchmarks.competitorAverages.leadTime
        ),
        status: getStatus(
          avgLeadTime,
        ...
        recommendation:
          avgLeadTime > 60
            ? "Optimize routing and reduce setup times"
            : undefined,
      },
      {
        id: "throughput",
        ...
        trend:
          throughputPerHour > benchmarks.competitorAverages.throughput
            ? "up"
            : "down",
        trendValue: Math.abs(
          throughputPerHour - benchmarks.competitorAverages.throughput
        ...
        status: getStatus(
          throughputPerHour,
          benchmarks.industryStandards.throughput
        ),
        description: "Number of orders processed per hour",
        recommendation:
          throughputPerHour < 10
            ? "Increase processing efficiency and reduce bottlenecks"
            : undefined,
      },
      {
        ...
        trend:
          ...
          benchmarks.competitorAverages.customerSatisfaction
            ? "up"
            : "down",
        trendValue: Math.abs(
          customerSatisfaction -
        ...
        status: getStatus(
          customerSatisfaction,
          benchmarks.industryStandards.customerSatisfaction
        ),
        description: "Customer satisfaction based on rush order performance",
        recommendation:
          customerSatisfaction < 80
            ? "Prioritize VIP customers and rush orders"
            : undefined,
      },
    ];
  ...
  const performanceInsights = useMemo((): PerformanceInsight[] => {
    ...
    const utilizationMetric = kpiMetrics.find(
      (m) => m.id === "resource-utilization"
    );
    ...
    if (onTimeMetric && onTimeMetric.status === "poor") {
      insights.push({
        ...
        description: `On-time delivery rate is only ${onTimeMetric.value.toFixed(
          1
        )}%, well below industry standards`,
        impact: "high",
        actionable: true,
        recommendation:
          "Immediate action needed: Review scheduling processes and capacity allocation",
      });
    }
    ...
    if (utilizationMetric && utilizationMetric.value < 60) {
      insights.push({
        ...
        description: `Department utilization is only ${utilizationMetric.value.toFixed(
          1
        )}%, indicating potential for higher throughput`,
        impact: "medium",
        actionable: true,
        recommendation:
          "Consider workload rebalancing or accepting more orders",
      });
    }
    ...
    if (utilizationMetric && utilizationMetric.value > 90) {
      insights.push({
        ...
        description: `Average utilization is ${utilizationMetric.value.toFixed(
          1
        )}%, which may lead to bottlenecks`,
        impact: "medium",
        actionable: true,
        recommendation:
          "Monitor for bottlenecks and consider capacity expansion",
      });
    }
    ...
    if (leadTimeMetric && leadTimeMetric.status === "excellent") {
      insights.push({
        ...
        description: `Average lead time of ${leadTimeMetric.value.toFixed(
          1
        )} minutes is industry-leading`,
        impact: "high",
        actionable: false,
      });
    }
    ...
    if (throughputMetric && throughputMetric.status === "excellent") {
      insights.push({
        ...
        description: `Processing ${throughputMetric.value.toFixed(
          1
        )} orders per hour exceeds industry benchmarks`,
        impact: "high",
        actionable: false,
      });
    }
    ...
    const bottleneckDept = gameState.departments.find(
      (d) => d.utilization > 95
    );
    if (bottleneckDept) {
      insights.push({
        ...
        description: `Department ${
          bottleneckDept.name
        } is at ${bottleneckDept.utilization.toFixed(1)}% utilization`,
        impact: "high",
        actionable: true,
        recommendation:
          "Consider redistributing workload or expanding capacity for this department",
      });
    }
      ...
      <div className="p-6 border-b border-gray-200">
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-sm text-gray-500">
            R09-R11: Advanced Analytics & Benchmarking
          </span>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex bg-gray-100 rounded-lg p-1">
            {(["current", "last-hour", "session", "all-time"] as const).map(
              (timeframe) => (
                <button
                  key={timeframe}
                  ...
                  <div className="grid grid-cols-4 gap-4 mb-4">
                    ...
                    <div className="text-center">
                      <p className="text-2xl font-bold text-gray-600">
                        {metric.id === "on-time-delivery" &&
                          benchmarks.competitorAverages.onTimeDeliveryRate.toFixed(
                            1
                          )}
                        {metric.id === "resource-utilization" &&
                          benchmarks.competitorAverages.utilizationRate.toFixed(
                            1
                          )}
                        {metric.id === "lead-time" &&
                          benchmarks.competitorAverages.leadTime.toFixed(1)}
                        {metric.id === "throughput" &&
                          benchmarks.competitorAverages.throughput.toFixed(1)}
                        {metric.id === "customer-satisfaction" &&
                          benchmarks.competitorAverages.customerSatisfaction.toFixed(
                            1
                          )}
                        {metric.unit}
                      </p>
                      ...
                      <p className="text-2xl font-bold text-yellow-600">
                        {metric.id === "on-time-delivery" &&
                          benchmarks.industryStandards.onTimeDeliveryRate
                            .average}
                        {metric.id === "resource-utilization" &&
                          benchmarks.industryStandards.utilizationRate.average}
                        {metric.id === "lead-time" &&
                          benchmarks.industryStandards.leadTime.average}
                        {metric.id === "throughput" &&
                          benchmarks.industryStandards.throughput.average}
                        {metric.id === "customer-satisfaction" &&
                          benchmarks.industryStandards.customerSatisfaction
                            .average}
                        {metric.unit}
            ...
            <div className="space-y-4">
              ...
              {performanceInsights.length === 0 && (
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm">
                    Your performance is tracking well across all metrics.
                  </p>
                </div>
              )}
            </div>
