      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">
            Manual Order Processing Game
          </h1>
          <p className="text-gray-600 mt-2">
            Click orders through each department step-by-step to learn
            manufacturing flow
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm border mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {departments.map((dept) => (
              <div
                key={dept.id}
                className={`p-4 rounded-lg border-2 ${dept.color}`}
              >
                ...
                <div key={order.id} className="border rounded-lg p-4">
                    ...
                    <div className="flex-1 mx-6">
                      <div className="flex items-center justify-center gap-2">
                        {order.route.map((deptId, index) => {
                          ...
                          const isCurrentStep =
                            index === order.currentStep && !isCompleted;
                  ...
                  <div className="mt-3 text-sm text-gray-600">
                    {isCompleted ? (
                      <span>
                        ✅ Order completed at{" "}
                        {order.completedTime?.toLocaleTimeString()}
                      </span>
                    ) : currentDept ? (
                      <span>
                        🔄 Currently at: {currentDept.name} (Dept{" "}
                        {currentDept.id})
                      </span>
                    ) : (
                    ...
                    {nextDept && !isCompleted && (
                      <span className="ml-4">
                        → Next: {nextDept.name} (Dept {nextDept.id})
                      </span>
                    )}
                  </div>
                </div>
        ...
        <div className="mt-6 bg-blue-50 rounded-lg p-6 border border-blue-200">
          ...
          <ol className="list-decimal list-inside space-y-2 text-blue-800">
            ...
            <li>
              Click the "Process" button for each order to move it through
              departments
            </li>
            <li>
              Follow the route shown for each order (arrows show the sequence)
            </li>
            <li>
              Watch how different routes affect processing time and coordination
            </li>
